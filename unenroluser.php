...
/**
 * Unenrol a user who was enrolled through a imsenterprise2 enrolment.
 *
 * Please note when unenrolling a user all of their grades are removed as well.
 *
 * @package    enrol
 * @subpackage imsenterprise2
 * @copyright  2011 Erik Ordway (based on the manual plugin)
...
require_capability("enrol/imsenterprise2:unenrol", get_context_instance(CONTEXT_COURSE, $course->id, MUST_EXIST));
...
$url = new moodle_url('/enrol/imsenterprise2/unenroluser.php', $returnurl->params());
$url->param('ue', $ueid);
...
if (!$plugin->allow_unenrol($instance) || $instance->enrol != 'imsenterprise2' || !($plugin instanceof enrol_imsenterprise2_plugin)) {
...
$message = get_string('unenroluser', 'enrol_imsenterprise2', array('user'=>fullname($user, true), 'course'=>format_string($course->fullname)));
$fullname = fullname($user);
$title = get_string('unenrol', 'enrol_imsenterprise2');
