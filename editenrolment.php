...
require_once("$CFG->dirroot/enrol/imsenterprise2/editenrolment_form.php");
...
// Make sure the user can manage imsenterprise2 enrolments for this course
require_capability("enrol/imsenterprise2:manage", get_context_instance(CONTEXT_COURSE, $course->id, MUST_EXIST));
...
$url = new moodle_url('/enrol/imsenterprise2/editenrolment.php', $returnurl->params());
...
if (!$plugin->allow_manage($instance) || $instance->enrol != 'imsenterprise2' || !($plugin instanceof enrol_imsenterprise2_plugin)) {
    print_error('erroreditenrolment', 'enrol');
}

$mform = new enrol_imsenterprise2_user_enrolment_form($url, array('user'=>$user, 'course'=>$course, 'ue'=>$ue));
...
$title = get_string('editenrolment', 'enrol_imsenterprise2');
